...
''why modify anything in here when its a mod loader??,,
-- Albi
*/
...
var txml = {
    ...
    register: function(type, f) {
        return this.registeredEvents.push({
            fire: f,
            type: type,
        });
    },
    debug: function(txt) {
        console.log("[TXML] "+txt);
    },
    customScenes: {
        Welcome: () => {
            return `Welcome to TXML v${txml.version}!\n
                    there example mod you can work with is there called mod.js if you want to develop
                    if you otherwise just wanna use mods that others created just paste them there!
                    if you look into your hotbar you can see theres a TXML menu you can use\n
                    Happy modding!\n\n{Continue|Menu|0}`;
        },
        Menu: () => {
            let cscenes = "";
            for (let cscene of Object.keys(txml.customScenes)) {
                if (!scene[cscene]) {
                    cscenes = cscenes+`\n{${cscene}|${cscene}|0}`;
                }
            }
            if (cscenes != "") {
                cscenes = "\n\nCustom Scenes:"+cscenes;
            }
            return `Menu\n{Start|Tutorial|0}${cscenes}\n\n{About|About|0}`;
        }
    },
    registerScene: function(name, f) {
        this.customScenes[name] = f;
    },
...
document.getElementById("TXMLButton").addEventListener("click", function() {
    const txmlmenu = document.getElementById("TXML");
    txmlmenu.style.display = (txmlmenu.style.display === "block") ? "none" : "block";
});
...
function SceneManager(selected) {
    if (txml.customScenes && selected in txml.customScenes) {
        var thescene = txml.customScenes[selected]();
    } else {
    ...
    for (const e of (txml.registeredEvents || [])) {
        if (e.type == txml.events.sceneChanged) {
            e.fire(selected);
        }
    }
}